  ...
  const handleDeleteBtn = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://my-assignment-10-server-two.vercel.app/deletedata/${id}`,
          {
            method: "DELETE",
          }
        )
          ...
          .then((data) => {
            if (data.deletedCount > 0) {
              ...
              const finalFilterData = filterData.filter(
                (dataFilter) => dataFilter._id !== id
              );
    ...
    <div className="card card-compact w-full  border bg-gray-50 py-10">
      ...
      <div className="card-body">
        <div className="flex gap-5">
          ...
          <button className="px-3 py-2 bg-gray-100 text-gray-800 font-semibold w-fit rounded-lg">
            {" "}
            {dataUserAll?.Stuck_status}{" "}
          </button>
        </div>
        ...
        <p className="flex gap-2 items-center ">
          ...
          {dataUserAll?.Customization}
        </p>
        <p className="flex gap-2 items-center ">
          <span className="text-black">
            <GiVibratingShield />
          </span>{" "}
          <span className="font-semibold"> Rating :</span> {dataUserAll?.Rating}
        </p>
        <div className="card-actions flex gap-3 ">
          <Link to={`/update/${dataUserAll?._id}`}>
            <button className="px-3 py-2 text-md border  text-black rounded-lg font-semibold my-5 flex gap-2 items-center">
              Update<span className="text-xl"> </span>
            </button>
          </Link>
          <Link>
            <button
              onClick={() => handleDeleteBtn(dataUserAll?._id)}
              className="px-3 py-2 text-md border  text-black rounded-lg font-semibold my-5 flex gap-2 items-center"
            >
